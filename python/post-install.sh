...
if command_exists pyenv; then
    ...
    PYTHON_3_LATEST_VERSION=$(pyenv install --list | sed 's/^  //' | grep -v - | grep -v 'dev\|a\|b\|rc' | grep '^3' | tail -1)
    PYTHON_2_LATEST_VERSION=$(pyenv install --list | sed 's/^  //' | grep -v - | grep -v 'dev\|a\|b\|rc' | grep '^2' | tail -1)
